  ...
  return (
    <div
      className={`relative h-4 ${
        currentTheme == "green" ? "bg-[#90C781]" : "bg-[#3E76FF]"
      } green:bg-green-500 rounded-t-lg  w-full`}
    >
      <div className="rounded-lg overflow-hidden shadow-lg w-full absolute top-2 left-0 bg-white">
        <div className="relative overflow-x-auto shadow-sm sm:rounded-lg">
          <div className="flex items-center justify-between py-4 px-2 sm:px-8  ">
            <p
              className={`${
                isSearchExpand ? "hidden" : "visible"
              } sm:text-lg text-[#323A46]`}
            >
              Cryptocurrency{" "}
            </p>
            <div
              className={`border border-[#BDBCBC] rounded-lg p-2 ${
                isSearchExpand ? "hidden" : "visible"
              } sm:hidden`}
            ...
            >
              <MagnifyingGlassIcon
                className={`h-5 w-5  ${
                  currentTheme == "green" ? "text-[#90C781]" : "text-[#3E76FF]"
                }`}
              />
            </div>
            ...
            >
              ...
              <input
                type="text"
                id="table-search"
                className="block text-sm p-2 pl-10 text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                name="search"
                ref={inputRef}
                onBlur={onLoseFocus}
                placeholder="Search"
          ...
          <div
            className={`flex font-semibold py-3.5 border border-[#EFF4FF] sm:px-8 ${
              currentTheme == "green" ? "text-[#90C781]" : "text-[#3E76FF]"
            }`}
          >
            <div className="flex-none w-12 sm:hidden"></div>
            <div className="flex-1 w-32 sm:flex-none ">Logo</div>
            <div className="flex-1 w-64 grow sm:flex-none sm:w-5/12 ">Name</div>
            <div className="hidden flex-1 w-32 sm:flex-none sm:block sm:w-72">
              Current Price
            </div>

            <div className="hidden flex-1 w-32 sm:block">Market Cap</div>
          </div>
          {data.map((item, index) => (
            <CollapsibleRow
              name={item.name}
              ...
              className={`p-3.5 border ${
                currentTheme == "green"
                  ? "hover:bg-[#EDFFE8]"
                  : "hover:bg-[#EFF8FF]"
              } border-[#EFF4FF]  sm:px-8`}
              idx={index}
            />
          ))}
        </div>
        <div
          className={`flex justify-between items-center text-xs my-4 mx-4  font-semibold sm:justify-end sm:gap-32 ${
            currentTheme == "green" ? "text-[#90C781]" : "text-[#3E76FF]"
          }`}
        >
          <div className="flex justify-between">
            ...
            <div
              ...
              data-te-dropdown-position="dropup"
            >
              <button
              ...
              >
                <div className="flex justify-between items-center">
                  <div className="sm:mr-2">{perPage}</div>
              ...
              {isDropdownOpen ? (
                <div className="absolute bottom-6 bg-white shadow-lg z-40 ">
                  <ul>
                    <li
                      className="border-1 px-4 py-2 hover:bg-gray-200"
